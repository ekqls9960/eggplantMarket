...
public class KakaoController {
	...
	public String readyForKakaoPay(@PathVariable("id") Long itemId, HttpSession session) {
		...
		ResponseEntity<String> kakaoPayResponse = restTemplate.exchange("https://kapi.kakao.com/v1/payment/ready",
				HttpMethod.POST, kakaoPayRequest, String.class);
	...
	public String kakaoPay(String pg_token, @PathVariable("id") Long itemId, HttpSession session, Model model) {
		...
		// exchange(요청 url, 전송 method, request, response return type)
		ResponseEntity<String> kakaoPayResponse = restTemplate.exchange("https://kapi.kakao.com/v1/payment/approve",
				HttpMethod.POST, kakaoPayRequest, String.class);
